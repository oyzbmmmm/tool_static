...
<?php
    ...
    // 顶栏 钻芒博客 文字自定义
    $zmki_name = new Typecho_Widget_Helper_Form_Element_Text('zmki_name', NULL, '静谧时光', _t('<svg  class="icon zmki_aliico" aria-hidden="true"><use xlink:href="#icon-prompt"></use></svg>  顶栏AD文字'), _t('输入你的首页顶栏收录提交右侧自定义文字，默认 钻芒博客'));
    $form->addInput($zmki_name);
    // 顶栏 钻芒博客 链接自定义
    $zmki_url = new Typecho_Widget_Helper_Form_Element_Text('zmki_url', NULL, 'https://www.silencetime.com/', _t('顶栏AD链接'), _t('输入你的首页顶栏收录提交右侧文字调整的url，默认 https://www.zmki.cn/'));
