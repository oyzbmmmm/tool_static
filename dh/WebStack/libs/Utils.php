...
class Utils {
    ...
    public static function json_decode($str,$flag){
...
EOF;
    ...
    public static function isLock($obj,$index=0){
        //todo 优化性能，普通游客在遇到加密分类直接返回 false，而不需要太多的判断
        $data=array();
        ...
        //没有 cookie并且没有设置区间时间
        if (!$cookie && @$content["start"] && @$content["end"]){//加密分类没有 cookie，禁止访问
        ...
        if (is_array($content)){//有 cookie需要进一步校验，没有 cookie说明是加密的
